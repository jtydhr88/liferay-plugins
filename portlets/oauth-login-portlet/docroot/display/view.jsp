...
%>
	...
	<c:choose>
		<c:when test="<%= Validator.isNull(socailAccountId) %>">
			...
			<a href="javascript:;" onClick="getAuthorizeURL(<%= oAuthConnection.getOAuthConnectionId() %>);"><%= LanguageUtil.format(pageContext, authorizeURLLinkMessage, HtmlUtil.escape(oAuthConnection.getName()), false) %></a>
		...
		<c:otherwise>
			...
			<%= LanguageUtil.format(pageContext, "you-have-bound-your-x-account", HtmlUtil.escape(oAuthConnection.getName()), false) %> <aui:a href="<%= unbindSocialAccountURL.toString() %>" label="unbind-your-account" />
